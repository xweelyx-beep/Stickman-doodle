<!-- Source: xweelyx-beep/Business .claude/rules/stickman.md @ 06de1d8
     Copied 2026-08-29. The original carries a `paths: channels/stickman/**`
     frontmatter that scopes it to a directory that does not exist in this
     repository; that frontmatter is the only thing removed. Body is verbatim.
     Business remains the source of truth — edit there, then re-copy. -->


# Stickman Channel — Canon (provisional)

Faceless 2D stickman animation explaining life skills and how things work, in
modern YouTube explainer style. Long-form is the engine; Shorts follow.

**This bible is deliberately incomplete.** Two episodes exist — one finished,
one nearly done — and neither has been analyzed, because they live on a local
Windows drive this environment cannot reach. Everything visual is therefore
**blocked, not guessed**. The sections marked `[BLOCKED]` get filled the moment
the operator provides a reachable link, using the same scene-by-scene analysis
that produced the Lilweid and Known Unknowns bibles.

**Do not invent a lock to fill a blocked section.** Two finished episodes
already define this channel's look. A lock invented here would contradict
footage that exists, which is worse than no lock at all.

---

## 1. What is decided

These came from the operator directly and are canon.

- **Subject:** life skills and how things work. Practical everyday explainers.
- **Form:** 2D stickman cartoon animation, modern YouTube explainer style.
- **Mascot:** **one recurring stickman**, the same figure in every episode. The
  channel's face, in a channel with no face.
- **Runtime shape:** long-form is the engine, Shorts are cut from it.
- **Narration:** AI voice, distinct from the other two channels. Each channel
  has its own narrator and they must not converge.
- **Faceless:** absolute. No real person, name or likeness, ever.

## 2. Episodes so far

Real, from the operator. Not a pattern to lock, just the record.
...
Both channels explain why people do things that hurt them. The operator names
**three** separations, all of which hold at once:

- **Mechanism versus meaning.** Stickman explains the biological and
  behavioural machinery. Lilweid explains the emotional meaning. Same human,
  different question.
- **Tone.** Stickman is light, quick, a little funny. Lilweid is slow and
  literary. Subjects may overlap because the feel never does.
- **Actionable versus reflective.** Stickman gives the viewer something to do
  about it. Lilweid deliberately does not.
...
**Head.** Large white rounded head, roughly a third of total height. Thick
black outline. Two big round eyes, white with solid black pupils, thin arched
black brows. Small closed smiling mouth. **No nose, no ears.**

**Hair.** Short, spiky, dark chocolate brown with a lighter brown highlight,
swept slightly forward, thick black outline.

**Limbs.** Thin solid black tubular arms and legs, constant width, no elbow or
knee joints, rubber-hose style. **This is the stick element.**

**Hands.** White four-fingered rounded gloves with black outlines, always
gloved, never bare.

**Wardrobe (default).** Royal-blue hooded sweatshirt with a hood, front kangaroo
pocket, drawstring, white inner collar V. Dark charcoal-grey trousers. White
low-top sneakers with grey soles and laces.

**Rendering.** Flat vector cel style, uniform heavy black outlines, light cel
shading, soft elliptical drop shadow under the feet, plain light-grey gradient
background.
...
the installed `scriptwriting` skill appends `"You're welcome."` by default, so
using that skill unmodified here will produce the wrong sign-off.

**Narration tool.** `[TBD]`. The operator prefers a free, non-robotic path
first, with ElevenLabs as fallback and a voice chosen per channel. Per the
...
These rules were written by the operator on an earlier project and are the real
working method. They apply to any channel, and they exist to stop credit burn.

- **Block structure.** 15-second blocks, three 5-second clips per block.
- **Mandatory halt.** Stop and wait for approval after every block. No
  exceptions.
- **Concurrency cap.** Never output more than three generation prompts at once.
- **No bulk generation.** Prompting a whole script at once is forbidden.
- **Camera motion is required on every clip**, exactly one of: `slow push-in`,
  `slow pull-back`, `slow tilt-up`, `gentle drift`. A static shot is a failure.
- **No block repeats the same motion in all three scenes.**
- **A style key is appended to every scene prompt**, identically.
- **Runtime is agreed before writing**, and the credit cost is projected per
  block before any submission.
- **Negative prompt on every clip**, listing what must not appear.

## 10. Production rules for this channel

Partial. Extend once footage is analyzed.
...
## 10. What unblocks this file

One reachable link to either episode. Unlisted YouTube, or any host that serves
a public or direct URL. On arrival: run the scene analysis, extract sections 5
through 8, and replace every `[BLOCKED]` marker with a verified lock.
