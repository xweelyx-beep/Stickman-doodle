# Revision changelog — character density pass

**File:** `prompts/image_prompts_why_you_check_your_phone_v2.txt`
**Base:** `prompts/image_prompts_why_you_check_your_phone.txt` (unmodified, kept for diffing)
**Episode:** Why You Check Your Phone · 11:22 · 157 frames
**Passes:** first half 2026-08-29, second half 2026-08-29

---

## Result — full 157-frame file

| | Base | v2 |
|---|---|---|
| Frames | 157 | 157 |
| Protagonist present | 45 | **75** |
| Presence | **28.7%** | **47.8%** |
| Frames modified | — | 30 |
| Frames byte-identical | — | **127** |

Target was 45–50%. 47.8% lands mid-band.

The 28.7% baseline matches the 29% given in the brief. An earlier pass measured
20.4%, which was correct for the 54 frames available at the time and is
superseded by this figure now the file is complete.

## Presence by half

| | Frames | Base | v2 | Injected |
|---|---|---|---|---|
| First half — `[00:00]`–`[03:19]` | 54 | 11 (20.4%) | 25 (46.3%) | 14 |
| Second half — `[#055]`–`[#157]` | 103 | 34 (33.0%) | 50 (48.5%) | 16 |
| **Total** | **157** | **45 (28.7%)** | **75 (47.8%)** | **30** |

The second half already ran warmer than the first — 33% against 20.4% — because
the smartphone, savannah and closing segments are written around the character.
The dry stretches were the technical ones, exactly as the brief said.

## Dry runs addressed

Measured as runs of 3+ consecutive frames with no protagonist.

**First half**

| Run | Frames | Segment |
|---|---|---|
| `[00:32]`–`[00:44]` | 4 | phone, app icons, hourglass, brain machinery |
| `[01:01]`–`[02:22]` | **27** | Skinner lab, box, pigeon, variable-ratio diagrams |
| `[02:27]`–`[03:19]` | 12 | Schultz lab, dopamine, monkey, oscilloscope |

**Second half** (positions given as frame numbers)

| Run | Frames | Segment |
|---|---|---|
| `[#056]`–`[#059]` | 4 | dopamine diagrams, THE GAP |
| `[#062]`–`[#076]` | **15** | Fiorillo/Tobler, 100/0/50, WANTING, the dial, Sapolsky |
| `[#088]`–`[#098]` | **11** | Schüll, casino floor, slot cutaway, reels, near-miss |
| `[#100]`–`[#104]` | 5 | NOT EXCITEMENT, melting clock, the wallet |
| `[#106]`–`[#113]` | 8 | NO BOTTOM, SAME PULL, ENGINEERED DELAY, Harris |
| `[#115]`–`[#122]` | 8 | Gloria Mark, the decline graph, 47 SECONDS |
| `[#150]`–`[#153]` | 4 | THE LEVER, THE MAYBE, OLD CIRCUIT NEW LEVER |

Longest run in the file: **27 frames** before, **8 frames** after.

## Frames modified — first half

| Frame | Why the character was placed here | Clause removed |
|---|---|---|
| `[00:44]` | Turns the abstract 'MACHINERY' reveal into his realisation; carries the hook's question into the thesis. | `no characters in the scene` |
| `[01:11]` | Opens the 27-frame Skinner run with the viewer's surrogate; the box is introduced to him, not just shown. | `no characters in the scene` |
| `[01:17]` | 'SIMPLE' lands as his ease; sets the baseline his later confusion is measured against. | `no human characters in the scene` |
| `[01:25]` | He owns the cause-and-effect beat rather than the diagram explaining itself. | `no characters in the scene` |
| `[01:30]` | First crack in his understanding — the pigeon stops and he does not yet know why. | `no human characters in the scene` |
| `[01:42]` | 'UNPREDICTABLE' becomes a felt reversal; his objection is the viewer's. | `no characters in the scene` |
| `[01:44]` | Pulls him physically into the experiment at the moment the reward stops arriving. | `no human characters in the scene` |
| `[01:51]` | The 'SOMETIMES' beat is the episode's hinge; his recognition marks it. | `no characters in the scene` |
| `[02:02]` | Scale is legible against his height — the bar means something because he is dwarfed by it. | `no characters in the scene` |
| `[02:22]` | Closes the Skinner run by rhyming the pocket drift from [00:00]; the lever and the phone become one gesture. | `no characters in the scene` |
| `[02:47]` | Opens the 12-frame Schultz run with a reacting observer instead of a floating diagram. | `no characters in the scene` |
| `[02:52]` | 'WRONG' is the segment's reversal; he registers it so the correction has a face. | `no characters in the scene` |
| `[03:15]` | The flat line means nothing without someone expecting a spike; he supplies the expectation. | `no characters in the scene` |
| `[03:19]` | The payoff of the whole Schultz segment lands on him, matching his realisation beats elsewhere. | `no characters in the scene` |

## Frames modified — second half

| Frame | Why the character was placed here | Clause removed |
|---|---|---|
| `[#059]` | 'THE GAP' names the episode's core idea; he is present when it is first drawn. | `no characters in the scene` |
| `[#064]` | Opens the 15-frame Fiorillo/Tobler run; 100% certainty reads as his ease. | `no characters in the scene` |
| `[#066]` | The 50% coin is the segment's whole point; his reaction marks it. | `no characters in the scene` |
| `[#067]` | Scales the middle bar against a known height instead of leaving it floating. | `no characters in the scene` |
| `[#071]` | 'WANTING' is the mechanism being named; he registers it. | `no characters in the scene` |
| `[#074]` | The needle slam is the payoff of the 100/0/50 run and lands on him. | `no characters in the scene` |
| `[#090]` | Opens the 11-frame casino run by giving the floor a human scale to recede from. | `no characters in the scene` |
| `[#093]` | The Skinner-box inset is the episode's argument; he makes the connection visibly. | `no characters in the scene` |
| `[#096]` | The near-miss only works as a felt beat; without a reaction it is just a diagram. | `no characters in the scene` |
| `[#098]` | Closes the casino run with the trapped feeling the movement is about. | `no characters in the scene` |
| `[#102]` | Lost time needs someone losing it; his slack expression carries the Machine Zone. | `no characters in the scene` |
| `[#104]` | 'UNTIL IT RAN OUT' is the cost; he is the one paying it. | `no faces in the scene` |
| `[#107]` | The pull-to-refresh / slot-lever rhyme is the segment's thesis and needs his recognition. | `-` |
| `[#113]` | Six billion pockets includes his; scale reads against a figure. | `no characters in the scene` |
| `[#117]` | Opens the 8-frame Gloria Mark run; the method is shown to him, not just displayed. | `no characters in the scene` |
| `[#122]` | 47 seconds is the episode's hardest datum; it lands on him and scales the number. | `no characters in the scene` |

`-` means the frame carried no exclusion clause to remove; `[#108]` already
showed a mitten glove, so only the lock block was added.

## Method

Each modified frame received the **full character lock block verbatim** — no
shorthand — inserted directly after the front style anchor, followed by a
placement-and-reaction clause written for that beat, then the original scene
description unchanged.

Frames that read `no characters in the scene`, `no human characters in the
scene` or `no faces in the scene` had that clause removed where the character
was added; leaving it would put a direct contradiction in the prompt. That is
the only deletion either pass made, and it is logged per frame above.

Constraints held, verified programmatically across all 157 frames:

- Front style anchor present: **157 / 157**.
- End style lock present and terminal: **157 / 157**.
- One frame per line, blank line between entries: **0 multi-line entries**.
- File ends with a single newline, no trailing blank.
- The 127 unmodified frames: **byte-identical** between base and v2.
- Every modified frame keeps its original scene text intact after the injected
  clause, except the logged exclusion-clause removal. **0 integrity problems.**
- Supporting characters — Skinner, Schultz, Fiorillo, Tobler, Sapolsky, Schüll,
  Harris, Gloria Mark — keep their own distinct descriptions and were not
  altered. Neither were the prehistoric-variant frames, which already carry the
  character in his hide wrap.

## Placement rationale

The protagonist is a **reacting observer**, never a second explainer. He is
placed at a frame edge or in the lower foreground, scaled small against the
subject, and given one readable expression drawn from the vocabulary already in
the file — the same raised-brow tilt, uneasy frown, and wide-eyed realisation
the hook frames use. No new expression vocabulary was invented.

Four placements do structural work rather than filling a gap:

- **`[02:02]`** and **`[#067]`** — he stands at the base of a bar chart so the
  tall bar is scaled against a known height instead of floating.
- **`[02:22]`** — his far glove drifts toward his pocket without him looking,
  the exact gesture from `[00:00]`. It closes the Skinner run by rhyming the
  lever with the phone.
- **`[#108]`** — he kneels at the pull-to-refresh frame and notices the slot
  lever overlaid on his own gesture. That rhyme is the segment's thesis.
- **`[#122]`** — he stands at the foot of the enormous "47", which both scales
  the number and puts the episode's hardest datum on a face.

## Open — the timestamps on frames 55–157

The second-half source arrived with **every timestamp field empty**. The scene
text is complete and was saved verbatim; only the `[MM:SS]` tokens were stripped
in transit, the same formatting loss that truncated the first paste.

Those 103 frames therefore carry **sequence markers** — `[#055]` through
`[#157]` — not timestamps. The markers are deliberately shaped so they cannot be
mistaken for a time: no colon, a leading `#`.

**Nothing was interpolated.** The first 54 frames have irregular, content-driven
gaps — 2 s, 3 s, 4 s, 5 s, 12 s — so there is no pattern to extend, and inventing
157 timings for a file that drives a shot render and maps to
`audio/music_bed_cues.md` would put plausible-looking wrong data where obviously
missing data is safer.

Supplying the real timestamps is a mechanical find-and-replace on the marker
column in both files. The segment boundaries already known from the music bed:

| Segment | Range |
|---|---|
| The Prediction Gap | `[03:46]`–`[05:37]` |
| Casino & Machine Zone | `[05:38]`–`[06:44]` |
| Friction & Gloria Mark | `[06:45]`–`[09:48]` |
| Savannah Evolution | `[09:49]`–`[10:39]` |
| Synthesis & Resolution | `[10:40]`–`[11:22]` |
